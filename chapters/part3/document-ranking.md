...
In its simplest form, document ranking is the task of assigning a score to a query-document pair.
...
presented as either binary classification or regression, where our labels are continuous relevance
scores between 0 and 1.

There are several different ranking tasks (ad-hoc, retrieval, clickthrough prediction, etc), as well
as several large datasets for it such as [MS-MARCO](https://microsoft.github.io/msmarco/) and
[ClueWeb](https://lemurproject.org/clueweb09/). Since these datasets tend to be rather large, we'll
use the [MIMICS](https://github.com/microsoft/MIMICS) query clarification dataset as a lightweight
alternative. Each row contains a query, a clarifying question, and a list of options presented to
the user. This is [used in
...
our input is a text document, which we process using a `TextField`. Now, we have an input that
contains multiple text documents (which can each be multiple words). In this case, we'll turn to the
...
indexed. We also have multiple labels. If we wanted to, we could use a `ListField[LabelField]` here,
...
This is a better fit for our dataset. Here's an overview of our `text_to_instance` method; for the
full dataset reader, see the code
...
be used to calculate the loss and metrics. We'll make a `Registerable` for this so we can try out a
...
We have a module to score each query/document pair, but our data is formatted as `(tokens,
options)`. We need to align the query to each option for the relevance layer to work.
