...
<pre data-line="7-10,11-15,28-33,35-39" class="language-js"><code class="language-js">local bert_model = "bert-base-uncased";
...
1. Use a `PretrainedTransformerTokenizer` (`"pretrained_transformer"`) for tokenizing words
2. Use a `PretrainedTransformerIndexer` (`"pretrained_transformer"`) for indexing words (BERT uses word pieces internally)
3. Replace the `TokenEmbedder` with a `PretrainedTransformerEmbedder` (`"pretrained_transformer"`) for embedding tokens
4. Replace the `Seq2VecEncoder` with a `BertPooler` (`"bert_pooler"`) for extracting the embedding for the `[CLS]` token

Also note that we switched the optimizer to use `BertAdam` with a recommended hyperparameter setting.
...
If you have multiple GPUs, you can also give a list of GPU IDs to use:
...
This will run a data parallel training across multiple GPUs and the loss will be aggregated and back-propagated after each forward pass. When we tried this on the BERT text classifier with four GPUs, we got an approximately 2x speedup (it's not exactly proportional to the number of GPUs due to synchronization and overhead). 
