...
from allennlp_semparse.state_machines.states import (
    GrammarBasedState,
    GrammarStatelet,
    RnnStatelet,
)
...
class NlaSupervisedParser(Model):
    def __init__(
        ...
        self._action_embedder = Embedding(
            num_embeddings=len(self._nla_world.all_possible_productions()),
    ...
    def _get_initial_grammar_statelet(self):
        ...
        action_mapping = {
            action: i
            for i, action in enumerate(self._nla_world.all_possible_productions())
        }
        ...
        for key, action_strings in nonterminal_keyed_actions.items():
            ...
            action_indices = [
                action_mapping[action_string] for action_string in action_strings
            ]

            action_input_embeddings = self._action_embedder(
                torch.tensor(action_indices)
            )
        ...
        initial_grammar_statelet = GrammarStatelet(
            [START_SYMBOL], translated_valid_actions, self._nla_world.is_nonterminal
        )
    ...
    def _get_initial_rnn_state(self, input_: Dict[str, torch.LongTensor]):
        ...
        final_encoder_output = util.get_final_encoder_states(
            encoder_outputs, input_mask, self._encoder.is_bidirectional()
        )
        memory_cell = encoder_outputs.new_zeros(
            batch_size, self._encoder.get_output_dim()
        )
        attended_input, _ = self._decoder_step.attend_on_question(
            final_encoder_output, encoder_outputs, input_mask
        )
    ...
    def _get_action_strings(
        self, action_sequence_indices: List[List[int]]
    ) -> List[List[str]]:
    ...
    def forward(
        ...
        initial_grammar_state = [
            self._get_initial_grammar_statelet() for i in range(batch_size)
        ]

        production_rules = [
            [
                ProductionRule(action, is_global_rule=True)
                for action in self._nla_world.all_possible_productions()
            ]
        ] * batch_size
        ...
        if not self.training:
            ...
            predicted_expressions = [
                self._nla_world.action_sequence_to_logical_form(sequence)
                for sequence in predicted_action_strings
            ]

            if target_action_sequences is not None:
                target_index_lists = [
                    [int(x) for x in instance_indices]
                    for instance_indices in target_action_sequences.squeeze(1)
                    .detach()
                    .cpu()
                    .numpy()
                ]
