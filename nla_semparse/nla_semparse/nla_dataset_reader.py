...
class NlaDatasetReader(DatasetReader):
    def __init__(
        ...
        **kwargs
        ...
        self._input_token_indexers = input_token_indexers or {
    ...
    def text_to_instance(
        self, input_expression: str, target_expression: str
    ) -> Instance:
        ...
        all_productions_field = ListField(
            [LabelField(label) for label in all_productions]
        )
        production_indices = {
            production: i for i, production in enumerate(all_productions)
        }
        ...
        target_action_sequence = self._nla_language.logical_form_to_action_sequence(
            target_expression
        )
        target_action_index_fields = [
            IndexField(production_indices[action], all_productions_field)
            for action in target_action_sequence
        ]
        target_action_field = ListField(target_action_index_fields)
        return Instance(
            {
                "input_expression": input_field,
